  ...
  return (
    <div className="space-y-12 py-4">
      <SectionHeading
        ...
        description="Halaman ini menampung sejarah, struktur, data dasar, dan potensi Padukuhan Kalipakel sebagai lokasi pengabdian KKN, agar website tidak hanya berisi galeri, tetapi juga memberi konteks yang kuat."
      />

      <section className="grid gap-6 lg:grid-cols-2">
        <article className="rounded-3xl border border-[color:var(--border)] bg-white/80 p-6 shadow-sm">
          <h3 className="text-xl font-semibold">Sejarah Singkat</h3>
          ...
          <p className="mt-3 text-sm leading-7 text-[color:var(--muted)]">
            ...
            berbatasan dengan Desa Parangtritis di sisi selatan (kawasan
            pesisir selatan Yogyakarta), dan berjarak kurang lebih 17 km dari
            pusat Kabupaten Bantul.
          </p>
        </article>
        <article className="rounded-3xl border border-[color:var(--border)] bg-[color:var(--surface)] p-6 shadow-sm">
          <h3 className="text-xl font-semibold">Struktur Pemerintahan</h3>
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        {demographics.map((item) => (
          <article
            key={item.label}
            className="rounded-3xl border border-[color:var(--border)] bg-white/75 p-6 shadow-sm"
          >
      ...
      <section className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
        <article className="rounded-3xl border border-[color:var(--border)] bg-[linear-gradient(135deg,rgba(15,118,110,0.12),rgba(255,250,242,0.92))] p-6 shadow-sm">
          <h3 className="text-xl font-semibold">Potensi Desa</h3>
          <p className="mt-4 text-sm leading-7 text-[color:var(--muted)]">
            Kalipakel dikenal dengan tradisi budaya Umbul Donga, sebuah
            upacara adat tahunan yang digelar di Sasono Loyo Kalipakel dengan
            kirab gunungan dari Lapangan Voli Kalipakel, serta gelar budaya
            pagelaran wayang kulit semalam suntuk sebagai bagian dari merti
            dusun. Di bidang ekonomi, warga menggerakkan UMKM binaan
            Kalurahan Donotirto seperti wedang uwuh, adrem, rempeyek, dan
            kerajinan kain perca (dompet, tas, pouch), yang pernah tampil
            dalam Bantul Creative Expo. Kelompok Wanita Tani Rukun Tani
            Kalipakel juga aktif dalam pelatihan pengolahan pangan lokal
            bersama Dinas Ketahanan Pangan dan Pertanian Kabupaten Bantul.
          </p>
        </article>
        <article className="rounded-3xl border border-[color:var(--border)] bg-white/80 p-6 shadow-sm">
          <h3 className="text-xl font-semibold">Catatan Pengisian</h3>
          <p className="mt-4 text-sm leading-7 text-[color:var(--muted)]">
            Data kualitatif (sejarah, struktur, potensi) di halaman ini
            disusun dari situs resmi Kalurahan Donotirto dan pemberitaan
            Pemkab Bantul. Data kuantitatif seperti jumlah KK, jumlah
            penduduk, dan rincian mata pencaharian per padukuhan belum
            tersedia secara terbuka di sistem data desa — tim KKN disarankan
            mengonfirmasi angka ini langsung ke Dukuh Kalipakel (Bapak
            Jayusman) atau kantor Kalurahan Donotirto saat penerjunan, lalu
            memperbarui nilai pada array <code>demographics</code> tanpa
            mengubah struktur desain.
          </p>
        </article>
      </section>
    </div>
